...
class AnalyticsEngine:
    ...
    async def get_simple_conversation_history(self, session_id: str, limit: int = 12) -> List[Dict]:
        ...
        try:
            ...
            if session_id in self.conversation_cache:
                ...
                cache_age_seconds = current_time - self.cache_timestamps.get(session_id, 0)
                ...
                if cache_age_seconds < (self.cache_ttl_hours * 3600):
                    ...
                    logger.info("✅ CACHE HIT", 
                            ...
                            hit_rate=round(self.cache_stats["hits"] / (self.cache_stats["hits"] + self.cache_stats["misses"]) * 100, 1) if (self.cache_stats["hits"] + self.cache_stats["misses"]) > 0 else 0)
                    
                    recent_messages = cached_messages[-(limit * 2):] if cached_messages else []
            ...
            logger.info("❌ CACHE MISS - Loading from KQL", 
                    session_id=session_id,
                    reason="not_in_cache" if session_id not in self.conversation_cache else "expired",
                    total_misses=self.cache_stats["misses"])
            ...
            if len(self.conversation_cache) > self.max_cache_sessions:
                oldest_session = min(self.cache_timestamps.items(), key=lambda x: x[1])[0]
            ...
            logger.error("💥 Cache error - falling back to KQL", 
                        session_id=session_id,
                        error=str(e),
    ...
    async def cached_intelligent_analyze(self, question: str, session_id: str = None, enable_ai_insights: bool = False, return_raw_data: bool = False) -> Dict[str, Any]:
        ...
        if is_contextual:
            ...
            asyncio.create_task(self.kql_storage.store_in_kql(question, raw_result, [], actual_session_id))
        ...
        if return_raw_data:
            return raw_result
        return await self._format_natural_response(question, raw_result)
    def _update_conversation_cache_inline(self, session_id: str, question: str, raw_result: Dict[str, Any]):
        ...
        try:
            ...
            if len(self.conversation_cache[session_id]) > 24:
                self.conversation_cache[session_id] = self.conversation_cache[session_id][-24:]
            
            # Update timestamp
            self.cache_timestamps[session_id] = time.time()
            self.cache_stats["updates"] += 1
            
            print(f"💾 Cache updated: {len(self.conversation_cache[session_id])} messages for session {session_id}")
    ...
    async def _load_conversation_from_kql(self, session_id: str, limit: int = 8) -> List[Dict]:
